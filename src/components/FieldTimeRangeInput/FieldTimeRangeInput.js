...
const FieldTimeRangeInput = props => {
    ...
    useEffect(() => {
        if(startTime) {
            ...
            if(_startDate.format("DD/MM/YYYY") === _endDate.format("DD/MM/YYYY")) {
    ...
    const updateTimeArr = (_startTime, _startDate, _endDate) => {
        let startTimeArr = [];
        let endTimeArr = [];
        ...
        if (endDate.format("DD/MM/YYYY") === startDate.format("DD/MM/YYYY")) {
            if (_startTime) {
                endTime = startDate.clone().set("hour", parseInt(_startTime.substring(0, 2))).set("minute", parseInt(_startTime.substring(3, 5))).add(timeRange, "minutes");
            }
            else {
